...
def questions_get_all(page):
    ...
    if page <= 1:
        ...
        query = query.limit(QUESTIONS_PER_PAGE).offset(
...
def questions_search(search_term):
    all_questions_in_db = Question.query.filter(
        Question.question.ilike(
            "%{}%".format(search_term))).all()
    all_questions = [question.format() for question in all_questions_in_db]
    return jsonify({"total_questions": len(all_questions_in_db),
                    "questions": all_questions,
                    "current_category": {}})  # Do we need this?
